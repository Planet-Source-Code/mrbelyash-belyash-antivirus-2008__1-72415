...
Begin VB.Form FrmNet 
   ...
   Begin ComctlLib.StatusBar StatusBar 
      ...
      BeginProperty Panels {0713E89E-850A-101B-AFC0-4210102A8DA7} 
         ...
         BeginProperty Panel2 {0713E89F-850A-101B-AFC0-4210102A8DA7} 
            ...
            Key             =   ""
            Object.Tag             =   ""
            Object.ToolTipText     =   "Сетевой монитор"
         EndProperty
         BeginProperty Panel3 {0713E89F-850A-101B-AFC0-4210102A8DA7} 
            AutoSize        =   2
            Key             =   ""
            Object.Tag             =   ""
            Object.ToolTipText     =   "Файловый монитор"
         EndProperty
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.CommandButton Command2 
         ...
         Style           =   1  'Graphical
         TabIndex        =   43
         ToolTipText     =   "Включить Net Shield"
         Top             =   180
         Width           =   765
      End
   ...
   Begin VB.Menu Menu 
      ...
      Begin VB.Menu monFile 
         Caption         =   "Включить монитор"
      End
      Begin VB.Menu ntsh 
         Caption         =   "Вкл Net Shield"
      End
...
If Command2.ToolTipText = "Включить Net Shield" Then
   Me.Timer1.Enabled = True
    Command2.ToolTipText = "Выключить Net Shield"
    ntsh.Caption = "Выключить Net Shield"
  '  URLMon.Active = True
    cmdMon_Click
...
Else
'включено
Command2.ToolTipText = "Включить Net Shield"
  'monFile.Caption = "Включить Net Shield"
...
'URLMon.StartMon txtMonitor
   ...
   ntsh.Caption = "Включить Net Shield"
...
On Error GoTo 100
Call ShellExecute(0, "Open", "mailto:" + "mrbelyash@rambler.ru" + "?Subject=" + "Проблемы при работе с Belyash Anti-Trojan", "", "", 1)
Exit Sub
100:
End Sub
...
If App.PrevInstance = True Then
 ...
 If countZapOLD = 0 Then
    MsgBox "Отсутствуют базы", vbCritical
    End
 End If
 '=========
...
n1 = CStr(GetSetting("Belyash AV", "Options", "Monitoring", "True"))
 If n1 = "True" Then
    Call setMonitor
    FillProcessListNT 'включаем мониторинг
   ' ModuleS.registry_mon 'мониторим реестр
    'Call cmdStartWatch 'включить слежение за каталогом винды
 Else
    tmrON = False
   ' Call setMonitor
    'FillProcessListNT
...
aryMode = Array(, , , , "")
strQuestion = "Вы действительно хотите закрыть приложение ?"
...
End Sub
Sub le_refresh()
lbAuto.Caption = "Автозагрузка"
lbFon.Caption = "Загрузка в фоновом режиме"
Check6.Caption = "Выдавать тултип"
Check1.Caption = "Ограничить размер лога"
Option1.Caption = "Перезаписывать"
Option2.Caption = "Обрезать"
Check3.Caption = "Перемещать в карантин"
ldMonik.Caption = "Запоминать состояние"
chkBlock.Caption = "Блокер приложений"
Check4.Caption = "Мониторить реестр"
chkDisk1.Caption = "Мониторить дисковую активность"
Check5.Caption = "Мониторить системный каталог"
chkmonNet.Caption = "Запоминать состояние"
chkmonSyte.Caption = "Мониторить опасные сайты"
chkTrojan.Caption = "Мониторить троянские порты"
...
If yesBlockSyte(URL) = True Then
    FrmNet.message 1, "Найден заблокированный сайт" + vbCrLf + nameBlockSyte
End If
...
For i = 0 To GetEntryCount
    ...
    Set Item = ListView1.ListItems.Add(, , "Неизвестное")
    Item.SubItems(5) = ""
    Else
...
End If
    ...
    myport = Chr(34) + arrport + Chr(34)
        If yesVirZ(myport) = True Then
              If namePort <> "" Then
                message 1, "Обнаружено соединение на троянский порт: " + Connection(i).LocalPort + vbCrLf + "Троян: " + namePort + vbCrLf + "Приложение: " + CStr(ListView1.ListItems(i))
...
BlockedProg = False
Else
'MsgBox "Нашел====" + FrmNet.Data1.Recordset("name") + "====" + FrmNet.Data1.Recordset("CRC")
...
Select Case Button
    ...
    Case vbRightButton
    CloseCon.Caption = "Прервать соединение: " & Right(Connection(ListView1.SelectedItem.Tag).FileName, Len(Connection(ListView1.SelectedItem.Tag).FileName) - InStrRev(Connection(ListView1.SelectedItem.Tag).FileName, "\"))
    TermProg.Caption = "Убить: " & Right(Connection(ListView1.SelectedItem.Tag).FileName, Len(Connection(ListView1.SelectedItem.Tag).FileName) - InStrRev(Connection(ListView1.SelectedItem.Tag).FileName, "\"))
...
If hostname = "" Then
StatusBar.Panels(1).Text = "Не могу достучаться до хоста"
Exit Sub
End If
...
StatusBar.Panels(1).Text = "Убит процесс: " & Right(Connection(ListView1.SelectedItem.Tag).FileName, Len(Connection(ListView1.SelectedItem.Tag).FileName) - InStrRev(Connection(ListView1.SelectedItem.Tag).FileName, "\"))
Else
StatusBar.Panels(1).Text = "Ошибка терминирования: " & Right(Connection(ListView1.SelectedItem.Tag).FileName, Len(Connection(ListView1.SelectedItem.Tag).FileName) - InStrRev(Connection(ListView1.SelectedItem.Tag).FileName, "\"))
End If
200:
End Sub
...
MsgBox "Вы не указали размер отчета", vbCritical, pr
Exit Sub
End If
...
'        lblMonitor.ForeColor = &HC0&
        Command7.ToolTipText = "Включить монитор"
        
        monFile.Caption = "Включить монитор"
        ' Frame3.Visible = True
    ...
    Else
        ...
        Call uc7Interval_Clicked
         Me.StatusBar.Panels(3).Text = "ON"
        start_regMon 'включаем слжение за реестром
        Command7.ToolTipText = "Выключить монитор"
        monFile.Caption = "Выключить монитор"
    '     Frame4.Visible = True
     '   Frame3.Visible = False
       
        monFolder
        'cmdBegin 'наблюдаем за каталогом винды
        tmrON = True
    End If
...
Dim Status As Long, retVal As Long
  ...
  Select Case Status
    ...
    Case 0 'To 2
      ...
      message 2, "Обнаружены изменения в каталоге " + vbCrLf & path(Status) & _
        vbCrLf & Time & vbCrLf
    Case WAIT_FAILED
...
If Dir$(App.path + "\updater.exe", vbNormal) = "" Then
    MsgBox "В каталоге с программой отсутствует модуль обновления." + vbCrLf + " Переустановите программу", vbCritical, pr
    Exit Sub
Else
...
Exit Sub
ErrorMessage:
MsgBox "Ошибка при запуске обновления-" + Error$
LogPrint "Ошибка запуска обновлялки-" + Error$
...
'Debug.Print "start registry mon"
    ...
    If Not lRet = TIME_OUT Then
        ...
        Me.message 2, "Обнаружено изменение в реестре.Заблокировано"
        modRegistry.CleanReg
        mRegNotifyChange.RegFindNextChange f_lChange, f_lKey, True, REG_NOTIFY_CHANGE_ALL
    End If
Exit Sub
10:
LogPrint "Ошибка слежения за реестром-" + Error$
End Sub
'
'---------------------------------------------------------------------------------------
