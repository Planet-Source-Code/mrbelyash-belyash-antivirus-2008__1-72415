...
Dim d As Variant
    d = Format(Now, "ddMMSSHHMMSS")
                If Dir$(sFile.Name + "." + d) <> "" Then
                    GoTo 101 'если есть такой файл в карантине то генерируем новое имя
                End If
...
errKill:
         ...
         If yesVir_old(Old_CRC) Then
             ...
             If MsgBox("Подозрение на вирусную активность, характерную для " + vbCrLf + nameVir_old + vbCrLf + "Блокировать ?", vbCritical + vbYesNo, pr) = vbYes Then
                'Process_Kill (proccAidi)
                FrmNet.message 2, "Заблокирован подозрительный процесс,характерный для" + vbCrLf + nameVir_old + vbCrLf + sFile
                LogPrint "Заблокирован подозрительный процесс,характерный для-" + nameVir_old + vbCrLf + sFile
            Else
                FrmNet.message 2, nameVir_old + vbCrLf + "Проигнорировано" + vbCrLf + sFile
                LogPrint nameVir_old + vbCrLf + "-проигнорировано" + vbCrLf + sFile
            End If
...
Public Function ScanFilepR(ByVal sPath As String, ByVal mi_file As String, proccAidi As Long) As Boolean
    'чисто для стартовой формы,чтобы не мешать сюда главную форму
    On Error Resume Next
    ScanFilepR = False
    'FrmNet.lblCount.Caption = Int(FrmNet.lblCount.Caption) + 1
     Dim fso As New FileSystemObject
...
'MsgBox ""
                'add to log
                 ...
                 ' If MsgBox("В памяти обнаружен вирус." + vbCrLf + nameVir + vbCrLf + "Блокировать ?", vbCritical + vbYesNo + vbSystemModal, pr) = vbYes Then
                  'End If
                      ' FrmNet.lblZabl.Caption = Int(FrmNet.lblZabl.Caption) + 1
                            'Call KillProcess(proccAidi)
                          '  FrmNet.Text1.Text = "В памяти обнаружен вирус." + vbCrLf + nameVir
                            'Call FrmNet.message
...
'                        LogPrint mi_file + "-terminate virus process"
         ...
         If yesVir_old(Old_CRC) Then
             ...
             If MsgBox("Подозрение на вирусную активность, характерную для " + vbCrLf + nameVir_old + vbCrLf + "Блокировать ?", vbCritical + vbSystemModal + vbYesNo, pr) = vbYes Then
                Process_Kill (proccAidi)
                FrmNet.message 2, "Заблокирован подозрительный процесс,характерный для" + vbCrLf + nameVir_old
                LogPrint "Заблокирован подозрительный процесс,характерный для-" + nameVir_old
            Else
                FrmNet.message 2, nameVir_old + vbCrLf + "Проигнорировано"
                LogPrint nameVir_old + vbCrLf + "-проигнорировано"
            End If
...
fil1.Move (App.path + "\quarantine\" + d)
    ...
    If Dir$(s12, vbNormal) = "" Or Dir$(s12, vbArchive) = "" Then
        ...
        FrmNet.message 1, "Вирус <" + virn1 + "> перемещен в карантин"
        
    Else
    LogPrint s12 + "-ошибка при перемещении в карантин"
            FrmNet.message 1, "Ошибка перемещения " + vbCrLf + virn1 + vbCrLf + " в карантин"
...
If Dir$(s13, vbNormal) = "" Or Dir$(s13, vbNormal + vbArchive) = "" Then
       ' FrmNet.lblDelete.Caption = Int(FrmNet.lblDelete.Caption) + 1
        LogPrint s13 + "-удален"
               ' FrmNet.Text1.Text = "Вирус <" + virn2 + ">  удален"
         FrmNet.message 1, "Вирус <" + virn2 + ">  удален"
    Else
            LogPrint s13 + "-ошибка при удалении"
          '  FrmNet.Text1.Text = "Ошибка при удалении " + vbCrLf + virn2
         FrmNet.message 3, "Ошибка при удалении " + vbCrLf + virn2
         Debug.Print Error$
    End If
Set fso = Nothing
    Set fil1 = Nothing
...
'проца записи перемещаемого в карантин
On Error GoTo 100
...
Select Case Err
Case 52
    MsgBox "Не правильное имя файла", vbCritical, pr
Case 53
    MsgBox "Файл лога не найден", vbCritical, pr
Case 54
    MsgBox "Не правильный режим файла", vbCritical, pr
Case 57
    MsgBox "Ошибка ввода/вывода", vbCritical, pr
Case 61
    MsgBox "Преполнение диска. Пора бы его почистить", vbCritical, pr
    'End
Case 62
    MsgBox "Почему-то произошела запись после закрытия файла", vbCritical, pr
Case 67
    MsgBox "Слишком много открытых файлов. Не могу я так работать", vbCritical, pr
    End
Case 68
    MsgBox "Не вижу я диск ", vbCritical, pr
    'End
Case 70
    MsgBox "Доступ к диску или каталогу запрещен", vbCritical, pr
    'End
Case 71
    MsgBox "Диск не готов", vbCritical, pr
Case 75
    
    If MsgBox("Ну не могу я работать с этим файлом или каталогом" & vbCrLf & ffile & vbCrLf & "Возможно это произошло из-за вмешательства антивируса. Продолжить ?" + vbCrLf + Error$, vbYesNo + vbCritical, pr) = vbNo Then
        End
    End If
Case 76
    MsgBox "Путь определен не верно...Возможно это произошло из-за вмешательства другой программы..." & ffile, vbCritical
    
Case 321
    MsgBox "Не правильный формат файла", vbCritical
'End
Case 3024
    MsgBox "Черт, не могу найти этот файл" & ffile, vbCritical, pr
'    End
Case 3176
    MsgBox "Не могу открыть файл" & ffile, vbCritical, pr
Case 3179
    MsgBox "Ошибочный конец файла", vbCritical, pr
'End
Case 3180
    MsgBox "Не могу записать в файл", vbCritical, pr
'End
Case Else
    MsgBox "Произошла внутренняя ошибка при открытии фала", vbCritical, pr
End Select
...
Case 75
    ...
    If MsgBox("Ну не могу я работать с этим файлом или каталогом" & vbCrLf & ffile & vbCrLf & "Возможно это произошло из-за вмешательства антивируса. Продолжить ?", vbYesNo + vbCritical, pr) = vbNo Then
        'End
    End If
Case 76
    MsgBox "Путь определен не верно...Возможно это произошло из-за вмешательства другой программы..." & ffile, vbCritical
...
If Dir$(App.path + "\mon.sec", vbNormal) = "" Then
    If MsgBox("В каталоге с программой отсутствует файл самопроверки.Возможно заражение программы." + vbCrLf + "Продолжить работу?", vbCritical + vbYesNo, "Опасность") = vbNo Then
        End
    Else
        GoTo 200
    End If
...
If Trim$(encFinal) <> Trim$(UCase$(maimCRC)) Then
    If MsgBox("Не совпадение контрольных сум. Возможно заражение." + vbCrLf + "Продолжить?", vbYesNo + vbCritical, "Опасность") = vbNo Then
        End
    End If
End If
200:
