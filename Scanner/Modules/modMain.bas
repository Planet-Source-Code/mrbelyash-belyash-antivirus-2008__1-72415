...
If ret = 0 Then
MsgBox "Ошибка получения версии Виндовс"
End
End If
...
GoTo 2
Case Else
MsgBox "Программа корректно работает только под Windows XP", vbExclamation
End
End Select
2:
End Sub
'startup sub
Sub Main()
Call SystemInformation 'проверим ХР это или нет
...
For iParam = LBound(sData) To UBound(sData)
    ...
    If sData(iParam) = "/md5" Then
        'вести лог md5 хешей проверяемых файлов
        mast_md5 = True
    End If
    If sData(iParam) = "/go" Then
        'не проверять при выборочном сканировании процессы
        checkmemory = False
    End If
    
        If sData(iParam) = "/ns" Then
        'не проверять стартапы
        nostartUP = False
    End If
If sData(iParam) = "/au" Then
        'не проверять автораны
        noAU = False
    End If
