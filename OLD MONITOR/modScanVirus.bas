...
'                LogPrint mi_file + "-found virus in memory"
                'add to log
                  ...
                  ' If MsgBox("В памяти обнаружен вирус." + vbCrLf + VSig(i).Name + vbCrLf + "Блокировать ?", vbCritical + vbYesNo + vbSystemModal, pr) = vbYes Then
                       frmTest.lblZabl.Caption = Int(frmTest.lblZabl.Caption) + 1
                            'Call KillProcess(proccAidi)
                            frmTest.Text1.Text = "В памяти обнаружен вирус." + vbCrLf + VSig(i).Name
...
fil1.Move (App.Path + "\quarantine\" + d)
    ...
    If Dir$(s12, vbNormal) = "" Then
        ...
        frmTest.Text1.Text = "Вирус <" + virn1 + "> перемещен в карантин"
        Call frmTest.message
    Else
    LogPrint s12 + "-ошибка при перемещении в карантин"
            frmTest.Text1.Text = "Ошибка перемещения " + vbCrLf + virn1 + vbCrLf + " в карантин"
        Call frmTest.message
    End If
Set fso = Nothing
    Set fil1 = Nothing
...
If Dir$(s13, vbNormal) = "" Then
        ...
        LogPrint s13 + "-удален"
                frmTest.Text1.Text = "Вирус <" + virn2 + ">  удален"
        Call frmTest.message
    Else
            LogPrint s13 + "-ошибка при удалении"
            frmTest.Text1.Text = "Ошибка при удалении " + vbCrLf + virn2
        Call frmTest.message
    End If
Set fso = Nothing
    Set fil1 = Nothing
...
Case 53
    MsgBox "Файл не найден", vbCritical, pr
Case 54
