...
'                LogPrint mi_file + "-found virus in memory"
                'add to log
                  ...
                  ' If MsgBox("Â ïàìÿòè îáíàðóæåí âèðóñ." + vbCrLf + VSig(i).Name + vbCrLf + "Áëîêèðîâàòü ?", vbCritical + vbYesNo + vbSystemModal, pr) = vbYes Then
                       frmTest.lblZabl.Caption = Int(frmTest.lblZabl.Caption) + 1
                            'Call KillProcess(proccAidi)
                            frmTest.Text1.Text = "Â ïàìÿòè îáíàðóæåí âèðóñ." + vbCrLf + VSig(i).Name
...
fil1.Move (App.Path + "\quarantine\" + d)
    ...
    If Dir$(s12, vbNormal) = "" Then
        ...
        frmTest.Text1.Text = "Âèðóñ <" + virn1 + "> ïåðåìåùåí â êàðàíòèí"
        Call frmTest.message
    Else
    LogPrint s12 + "-îøèáêà ïðè ïåðåìåùåíèè â êàðàíòèí"
            frmTest.Text1.Text = "Îøèáêà ïåðåìåùåíèÿ " + vbCrLf + virn1 + vbCrLf + " â êàðàíòèí"
        Call frmTest.message
    End If
Set fso = Nothing
    Set fil1 = Nothing
...
If Dir$(s13, vbNormal) = "" Then
        ...
        LogPrint s13 + "-óäàëåí"
                frmTest.Text1.Text = "Âèðóñ <" + virn2 + ">  óäàëåí"
        Call frmTest.message
    Else
            LogPrint s13 + "-îøèáêà ïðè óäàëåíèè"
            frmTest.Text1.Text = "Îøèáêà ïðè óäàëåíèè " + vbCrLf + virn2
        Call frmTest.message
    End If
Set fso = Nothing
    Set fil1 = Nothing
