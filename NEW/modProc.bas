...
Public Declare Function WriteProcessMemory Lib "kernel32" (ByVal hProcess As Long, lpBaseAddress As Long, lpBuffer As Long, ByVal nSize As Long, lpNumberOfBytesWritten As Long) As Long
...
Dim inList As Boolean
    ...
    Do While r
        ...
        If noList = False Then
            If refProc = True Then
                FrmNet.lvProcess.AddItem ProcessName ' & "=" & uProcess.th32ProcessID
                                   ' Call checkthis(uProcess.procName, uProcess.th32ProcessID)
            End If
        End If
    ...
    'FrmNet.lblProcRun.Caption = runningProc
End Sub
...
Dim newProc As String
    For i = 1 To UBound(copyArr)
        If copyArr(i) = 0 Then
            newProc = tempArr1(i)
            If InStr(1, newProc, "svchost.exe") > 0 Or InStr(1, newProc, "taskmgr.exe") > 0 Then
            'MsgBox ""
            Else

               ' MsgBox "новый процесс=" & newProc
                refProc = True
               ...
               ' SuspendThreads (procinfo(frmIndex).th32ProcessID)
                ...
                EnumWindows AddressOf EnumWindowsProc, ByVal 0&
                DoEvents
                'If MsgBox("пропустить", vbYesNo) = vbYes Then
...
Dim fpale As String
FillProcessListNT
For i = 1 To FrmNet.lstBox.ListCount - 1
d = InStr(1, Trim$(FrmNet.lstBox.List(i)), Trim$(a), vbTextCompare)
    If d <> 0 Then
     
    Thread_Suspend (hNumrer1)
     
         ' If FileorFolderExists(FrmNet.lstBox.List(i)) = False Then
            'ResumeThreads (hNumrer1)
            'Exit Sub
       ' End If
        
      If ScanFilepR("c:", FrmNet.lstBox.List(i), hNumrer1) = False Then
             Thread_Resume (hNumrer1)
            Exit Sub
        End If
        
        
                   fpale = modFileManipulation.GetMD5(CStr(FrmNet.lstBox.List(i)))
            If FrmNet.BlockedProg(fpale) = True Then
       
             ModlLoadProcess.KillProcessById (hNumrer1)
             FrmNet.message 1, "Блокирован запуск запрещенного приложения" + vbCrLf + FrmNet.lstBox.List(i)
...
Dim delProc As String
    For i = 1 To UBound(copyArr)
        'For q = 0 To FrmNet.List2.ListCount
         '   If tempArr1(i) = FrmNet.List2.List(q) Then
        '        MsgBox "Found"
         '   End If
        'Next q
        If copyArr(i) = 0 Then
...
For i = 1 To NumElements
    ...
    If hProcess Then
      ...
      'FrmNet.lstProcid1.AddItem lRet
        '
        ...
        If lRet <> 0 Then
            ...
            '
            FrmNet.lstBox.AddItem sModName
            ...
            Do
                ...
                If ChekLstExist(Trim(sChildModName)) = False Then
                        ...
                        FrmNet.lstBox.AddItem "    " & sChildModName
                    
                End If
                'FrmNet.lstProcid1.AddItem hProcess
                End If
            Loop
        End If
    Else
...
For i = 1 To FrmNet.lstBox.ListCount - 1
If Trim$(FrmNet.lstBox.List(i)) = q1 Then
    ChekLstExist = True
    Exit Function
End If
        ChekLstExist = False
...
Public Function FillProcessListNT_All() As Long
'
' Clears the listbox and fill it with the
' processes and the modules used by each process.
'
...
For i = 1 To NumElements
    ...
    If hProcess Then
        ...
        '
        lRet = EnumProcessModules(hProcess, ModuleS(1), 200, cbNeeded2)
      'frmAlert.lstProcid1.AddItem lRet
        '
        ' If the Module Array is retrieved, Get the ModuleFileName.
        '
        If lRet <> 0 Then
            ...
            ' Add the process to the listbox.
            '
            frmAlert.lstBox.AddItem sModName
            ...
            Do
                ...
                If ChekLstExist(Trim(sChildModName)) = False Then
                        ...
                        frmAlert.lstBox.AddItem "    " & sChildModName
                    
                End If
                'frmAlert.lstProcid1.AddItem hProcess
                End If
            Loop
        End If
    Else
...
'
'MsgBox iModDlls
FillProcessListNT_All = iProcesses
End Function
