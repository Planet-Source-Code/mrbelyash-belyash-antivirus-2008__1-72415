...
Private Declare Function GetThreadTimes Lib "kernel32.dll" (ByVal hThread As Long, ByRef lpCreationTime As FILETIME, ByRef lpExitTime As FILETIME, ByRef lpKernelTime As FILETIME, ByRef lpUserTime As FILETIME) As Boolean
...
Public Declare Function SetPriorityClass _
Lib "kernel32" (ByVal hProcess As Long, _
ByVal dwPriorityClass As Long) As Long


Public Sub SetPriority(Priority As Long)
Dim ProcId As Long 'ïåðåìåííàÿ äëÿ id ïðîöåññà
Dim ProcHandle As Long 'ïåðåìåííàÿ äëÿ handle ïðîöåññà
ProcId = GetCurrentProcessId 'ïîëó÷èì id
ProcHandle = OpenProcess(PROCESS_QUERY_INFORMATION, False, ProcId)
'ïîëó÷èëè Handle
Call SetPriorityClass(ProcHandle, Priority) ' ñòàâèì ïðèîðèòåò
End Sub
...
Public Sub Thread_Resume(T_ID As Long)
    'âîññòàíîâèòü çàìîðîæåíûé ïðîöåññ
    On Error GoTo VB_Error
